...
class HashLinkCommand extends Command {
  ...
  public function install() {
    ...
    if(Travix.isLinux) {
      // V1.5 only supports byte code generated by Haxe 4.0.0-preview.1, 4.0.0-preview.2 and 4.0.0-preview.3
      exec('wget', ['https://github.com/HaxeFoundation/hashlink/releases/download/1.5/hl-1.5.0-linux.tgz']);
      exec('tar', ['xvzf', 'hl-1.5.0-linux.tgz']);
      
      // workaround for "error while loading shared libraries: libhl.so cannot open shared object file: No such file or directory"
      Sys.putEnv("LD_LIBRARY_PATH", "hl-1.5.0-linux".fullPath());
      
      hlCommand = "hl-1.5.0-linux/hl";
      exec('chmod', ['u+x', hlCommand]);
      return;
    }
  }
